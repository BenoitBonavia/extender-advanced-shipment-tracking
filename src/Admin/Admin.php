...
final class Admin {
	 ...
	 */
	public const SETTINGS_TAB = 'east';
	...
	public function register(): void {
		...
		add_filter( 'plugin_action_links_' . EAST_BASENAME, array( $this, 'add_action_links' ) );
	...
	public function add_action_links( array $links ): array {
		$settings_link = sprintf(
			...
			esc_html__( 'Réglages', 'extender-advanced-shipment-tracking' )
		);
	...
	public function enqueue_assets( string $hook_suffix ): void {
		...
		wp_add_inline_script(
			'east-admin',
			'window.eastAdmin = ' . wp_json_encode(
				array(
					'ajaxUrl' => admin_url( 'admin-ajax.php' ),
					'nonce'   => wp_create_nonce( 'east_admin' ),
				)
			) . ';',
			'before'
		);

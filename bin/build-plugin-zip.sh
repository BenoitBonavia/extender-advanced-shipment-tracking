...
SLUG="extender-advanced-shipment-tracking"
...
CONST_VERSION="$(show_from_ref "$SLUG.php" \
	| grep -m1 "EAST_VERSION" \
	| sed -E "s/.*'EAST_VERSION',[[:space:]]*'([^']+)'.*/\1/")"
...
fail_mismatch() {
	...
	echo "  constante EAST_VERSION: $CONST_VERSION" >&2
...
if command -v unzip >/dev/null 2>&1; then
	...
	for required in \
		...
		"$SLUG/src/Integration/AdvancedShipmentTracking.php" \
		"$SLUG/src/Integration/Boxtal.php" \
	...
	if grep -q "^$SLUG/\(\.github\|bin\|stubs\)/" <<< "$LISTING"; then
		echo "ERREUR : .github/, bin/ ou stubs/ présents dans l'archive." >&2
		exit 1
	fi
fi
